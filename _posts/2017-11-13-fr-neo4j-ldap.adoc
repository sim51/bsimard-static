...
Celle-ci me permet de lancer un serveur OpenLDAP pré-configurer où :
  * le domaine par défault est  `dc=example,dc=org`
  * l'administrateur `cn=admin,dc=example,dc=org` (avec le password `admin`).

De plus je peux lui passer en paramètre le chemin d'accès d'un répertoire contenant des fichiers LDIF, qui vont être éxécutés à son lancement.

Ainsi, voici le fichier LDIF que j'ai réalisé pour créer la structure de mon LDAP
...
Après un redémarrage du service, la nouvelle configuration est prise en charge, et on peut tester la connection LDAP directement avec le navigateur Ne4j : http://localhost:7474/browser
...
Le **DN** est calculé par Neo4j grâce à la clef de configuration `dbms.security.ldap.authentication.user_dn_template`. Le `{0}` est remplacé par le login de l'utilisateur.
...
Ceci est délégué à un utilisateur système qui va effecuter une recherche dans le LDAP pour retrouver l'utilisateur et ses groupes associés.
...
Pour finir, la correspondance entre les groupes LDAP et les roles Neo4j est effectué en respectant les règles définie dans la clef de configuration **dbms.security.ldap.authorization.group_to_role_mapping**.

== En bref

 * Le connecteur LDAP est uniquement présent dans l'édition **Entreprise** de Neo4j
 * La configuratin est simple et permet de gérer ala fois l'authentification et les accès
